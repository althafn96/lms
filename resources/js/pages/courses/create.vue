            ...
            <form @submit.prevent="handleSubmit" ref="form" class="form fv-plugins-bootstrap5 fv-plugins-framework d-flex flex-column flex-lg-row">
                <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px mb-7 me-lg-10">
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body text-center pt-0">
                            ...
                            <!--end::Image input-->
                            <!--begin::Description-->
                            <div class="text-muted fs-7">Set the course thumbnail. Only *.png, *.jpg and *.jpeg image files are accepted</div>
                            <!--end::Description-->
                        </div>
                        <!--end::Card body-->
                    </div>
                ...
                <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                                ...
                                <div class="mb-10 fv-row d-flex flex-column">
                                    ...
                                    <div class="btn-group w-50" data-kt-buttons="true" data-kt-buttons-target="[data-kt-button]">
                                        ...
                                        <label class="btn btn-outline-secondary text-muted text-hover-white text-active-white btn-outline btn-active-success" data-kt-button="true">
                                            <!--begin::Input-->
                                            <input v-model="formData.is_featured" class="btn-check" type="radio" name="is_featured" v-bind:value="1" />
                                            <!--end::Input-->
                                            Yes
                                        </label>
                                        <!--end::Radio-->
                                        <!--begin::Radio-->
                                        <label class="btn btn-outline-secondary text-muted text-hover-white text-active-white btn-outline btn-active-danger active" data-kt-button="true">
                                            <!--begin::Input-->
                                            <input v-model="formData.is_featured" class="btn-check" type="radio" name="is_featured" checked="checked" v-bind:value="0" />
                                            <!--end::Input-->
                                            No
                                        </label>
                                        <!--end::Radio-->
                    ...
                    <div class="d-flex justify-content-end">
                        <!--begin::Button-->
                        <button @click.prevent="router.go(-1)" id="course_create_cancel" class="btn btn-light me-5">Cancel</button>
                        <!--end::Button-->
                        <!--begin::Button-->
...
import addCourse from '@/graphql/mutations/courses/addCourse.gql'
...
const { mutate: addCourseMutation, onDone, onError  } = useMutation(addCourse)
...
const handleSubmit = () => {
    ...
    validator.validate().then(async function (status) {
        if (status == 'Valid') {
            ...
            try {
                const res = await addCourseMutation(
                    formData,
                    {
                        context: {
                            hasUpload: true
                ...
                router.push(`/courses/edit/${res.data.createCourse.slug}`)
...
const handleLessonFilesInput = (e, moduleIndex, lessonIndex) => {
    console.log(lesson_files.value);
    let selectedLessonFiles = lesson_files.value[moduleIndex[lessonIndex]].files
