...
import updateBatch from '@/graphql/mutations/students/updateBatch.gql'
...
import getBatch from '@/graphql/queries/students/getBatch.gql'
...
page.breadcrumbs = [
    ...
    {link: '', title: 'Update Batch'}
]

const router = useRouter()

const { result, loading } = useQuery(getBatch, {
...
const { mutate: updateBatchMutation, onDone, onError  } = useMutation(updateBatch)
...
const handleSubmit = () => {
    ...
    validator.validate().then(async function (status) {
        if (status == 'Valid') {
            ...
            try {
                const res = await updateBatchMutation(
...
watchEffect(() => {
    if(result.value) {
        if(!result.value.batch) {
            ...
            formData.id = batch.id
            formData.name = batch.name
            formData.commences_on = batch.commences_on
            formData.completes_on = batch.completes_on
            formData.enrollments_closes_on = batch.enrollments_closes_on
            formData.alloted_count = batch.alloted_count
            formData.selected_location = batch.location == null ? null : {
                id: batch.location.id,
                name: batch.location.name
            }
            formData.selected_course = batch.course == null ? null : {
                id: batch.course.id,
