            ...
            <form @submit.prevent="handleSubmit" ref="form" class="form fv-plugins-bootstrap5 fv-plugins-framework d-flex flex-column flex-lg-row">
                <div class="d-flex flex-column gap-7 gap-lg-10 w-100 w-lg-300px mb-7 me-lg-10">
                    <div class="card card-flush py-4">
                        ...
                        <div class="card-body text-center pt-0">
                            <!--begin::Image input-->
                            <div :class="{'image-input-empty': !loading && !result.instructor.display_picture != ''}" class="image-input image-input-outline mb-3" data-kt-image-input="true" style="background-image: url(/media/svg/files/blank-image.svg)">
                                <!--begin::Preview existing Display Picture-->
                                <div class="image-input-wrapper w-150px h-150px" :style="!loading ? `background-image: url(${result.instructor.display_picture})` : ''"></div>
                ...
                <div class="d-flex flex-column flex-row-fluid gap-7 gap-lg-10">
                    ...
                    <div class="d-flex justify-content-end">
                        <!--begin::Button-->
                        <button @click.prevent="router.go(-1)" id="instructor_edit_cancel" class="btn btn-light me-5">Cancel</button>
                        <!--end::Button-->
                        <!--begin::Button-->
...
import updateInstructor from '@/graphql/mutations/users/updateInstructor.gql'
...
const page = usePage()
page.title = "Update Instructor"
page.breadcrumbs = [
    {link: '/dashboard', title: 'Dashboard'},
    {link: '/instructors', title: 'Instructors'},
    {link: '', title: 'Update Instructor'}
]

const router = useRouter()

const { result, loading } = useQuery(getInstructor, {
...
let instructor = reactive({})
let locations = ref([])

const { mutate: updateInstructorMutation, onDone, onError  } = useMutation(updateInstructor)
...
const handleSubmit = () => {
    validator.validate().then(async function (status) {
        if (status == 'Valid') {
            ...
            try {
                const res = await updateInstructorMutation(formData,{
                    context: {
                        hasUpload: true
                    }
                })
...
watchEffect(() => {
    if(result.value) {
        if(!result.value.instructor) {
            router.push('/instructors')
        } else {
            instructor = result.value.instructor
            formData.id = instructor.id
            formData.first_name = instructor.first_name
            formData.last_name = instructor.last_name
            formData.bio = instructor.bio
            formData.user.update.status = instructor.user.status ? 1 : 0
            formData.user.update.id = instructor.user.id
            formData.user.update.email = instructor.user.email

            if(instructor.locations.length) {
                let ins_locations = []
                instructor.locations.forEach(function(location) {
                    ins_locations.push({
                        id: location.id,
                        name: location.name
                    })
                ...
                formData.selected_locations = ins_locations
            }
        }
    }
})
